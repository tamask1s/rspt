...
#endif /* ZZLIB */
{
	...
	if (len > 64)
	{
		...
		printf("BLOCKTYPES: %d %d", block.type, block.mm_type);
		if (block.mm_type != 0)
		{
			GET_TSC(p1);
...
int ZzCompressBlock(unsigned char *buffer, unsigned int  size, unsigned int  compression_mode, unsigned int  multimedia_test)
{
	int len;
